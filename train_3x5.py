...
from model_3x5 import highwayNet
...
if __name__ == '__main__':
    ...
    args['d_model'] = 64
    args['n_head'] = 8
    args['num_encoder_layers'] = 7
    args['num_decoder_layers'] = 7
    ...
    trainEpochs = 20
    optimizer = torch.optim.Adam(net.parameters(), betas=(0.9, 0.98), lr=0.0001, eps=1e-9)  # lr = ...
    ...
    for epoch_num in range(trainEpochs):
        ...
        now = avg_val_loss / val_batch_count
        if now < best:
            best = now
            torch.save(net.state_dict(), 'trained_models/natr5.tar')
